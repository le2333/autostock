...
CREATE TABLE data_tracking (
    ...
    name VARCHAR,                    -- 股票名称
    -- 数据状态信息
    has_daily_data BOOLEAN DEFAULT FALSE,   -- 是否有详细交易数据
    data_start_date DATE,            -- 数据开始日期
    data_end_date DATE,              -- 数据结束日期
    last_sync_date DATE,             -- 最后同步日期
    data_quality VARCHAR DEFAULT 'good',    -- 数据质量（good、partial、poor）
    -- 管理信息
...
```sql
-- 示例：stock_000001_daily
CREATE TABLE stock_{symbol}_daily (
...
1. **项目初始化**
   ...
   - [ ] 创建完整的目录结构
   - [ ] 设置 `setup.py` 和 `requirements.txt`
   - [ ] 初始化 Git 仓库
   - [ ] 配置开发环境（虚拟环境）

2. **数据库层建设 (渐进式)**

   - [ ] 安装和配置 SQLModel + Alembic
   - [ ] 在 `database/models/` 中定义**第一批**核心模型 (如 `MarketOverview`)
   - [ ] 运行 `alembic init` 初始化迁移环境
   - [ ] 运行 `alembic revision --autogenerate` 创建首个迁移文件
   - [ ] 运行 `alembic upgrade head` 应用迁移，生成数据库表

3. **数据管理器核心功能**

   - [ ] 实现 `database/engine.py` 数据库引擎
   - [ ] 实现 `datamanager/session.py` 会话管理
